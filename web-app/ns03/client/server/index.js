...
app.all("/api/prv/*", function (req, res, next) {
  try {
    console.log(
      "Auth: " +
        getAuthentication(req.cookies) +
        " - Role: " +
        getRole(req.cookies) +
        " - User: " +
        getUser(req.cookies) +
        " - Email: " +
        getEmail(req.cookies)
    );
    next();
  } catch {
...
app.post("/api/prv/survey", jsonParser, async (req, res) => {
  try {
...
// Returns the list of all surveys from all users associated with boardID = :boardID
app.get("/api/prv/survey/boardID/:boardID", async (req, res) => {
  try {
    const ret = await surveys.getBoardIDSurveys(req.params.boardID);
    res.status(200).json(ret);
  } catch (error) {
    console.error("Error: /api/survey/boardID/:boardID - " + error.message);
    res.status(error.status || 500).json(error.message);
  }
});
...
app.post("/api/prv/personal", jsonParser, async (req, res) => {
  try {
